...
public class DTWClientAuto {
    ...
    public static void main(String args[]) throws Exception {
        ...
        try{
        	...
        	LinkedList<PriorityQueue<BigInteger>>[] randomSet = new LinkedList[s.getLength()];
        	for(int i = 0; i< randomSet.length; i++){
        		randomSet[i] = new LinkedList<PriorityQueue<BigInteger>>();
        	}
        		...
        		switch (number) {
        			...
        			case 3:
                            ...
                            if(serverDimension != s.getDimension()){
                                ...
                                randomSet = initialRandomSet(randomSet, serverSize, RND_SET);
        			...
        			case 5:
                            ...
                            for(int i = 1; i < dynamicMatrix.length; i++){
                                for(int j = 1; j < dynamicMatrix[i].size(); j++){
                                    ...
                                    receive(dis);
                                    timerBefore = System.nanoTime();
                                    BigInteger minRandom = paillier.Encryption(randomSet[i].get(j).peek().negate());
                                    outputString = candidatesGenerator(paillier, randomSet[i].get(j), dynamicMatrix[i-1].get(j), dynamicMatrix[i-1].get(j-1), dynamicMatrix[i].get(j-1)); 
                                    ...
                                    timerBefore = System.nanoTime();
                                    sc = new Scanner(inputString);
                                    BigInteger entry = new BigInteger(sc.next());
                                    BigInteger finalEntry = paillier.HomomorphicAddition(entry, minRandom);
                                    dynamicMatrix[i].set(j, paillier.HomomorphicAddition(finalEntry, distanceMatrix[i].get(j)));
                                    timerAfter = System.nanoTime();
                                    ClientPhase2Time += timerAfter - timerBefore;
                                }
                            }
        			...
        			case 6:
                            ...
                            System.out.println("$$ DTW Distance is: " + result + " $$\n");
                            flag = false;
                            outputString = "End Session.";
                            break;
        			default:
    ...
    private static LinkedList<PriorityQueue<BigInteger>>[] initialRandomSet(LinkedList<PriorityQueue<BigInteger>>[] randomSet, int rows, int RND_SET){
        for(int i = 0; i< randomSet.length; i++){
            for(int j = 0; j< rows; j++){
    ...
    //(Can be changed to parallel computing, no need)
    ...
    private static PriorityQueue<BigInteger> randomSetGenerator(int size, int RandomNumberLength){
        PriorityQueue<BigInteger> randomSet = new PriorityQueue<BigInteger>();
    ...
    //(Can be changed to parallel computing)
    private static String candidatesGenerator(PaillierEncryptOnly p, PriorityQueue<BigInteger> randomSet, BigInteger a, BigInteger b, BigInteger c){
        String candidates = "";
